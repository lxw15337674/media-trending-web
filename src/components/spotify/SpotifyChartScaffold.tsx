...
import type { SpotifyCountryOption } from '@/lib/spotify/types';
import { createRegionDisplayNames } from '@/lib/youtube-hot/labels';

interface SpotifyChartScaffoldMessages {
  title: string;
  subtitle: string;
...
interface SpotifyChartScaffoldProps {
  locale: Locale;
  t: SpotifyChartScaffoldMessages;
  country: string;
...
export function SpotifyChartScaffold({
  locale,
  t,
  country,
  countries,
  ...
  const updateCountry = (nextCountry: string) => {
    const normalizedCountry = normalizeSpotifyCountryCode(nextCountry);
