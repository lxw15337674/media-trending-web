...
import { AppStoreGameChartsClient } from '../src/lib/app-store-games/crawler';
import { saveAppStoreGameChartSnapshot } from '../src/lib/app-store-games/db';
import {
  APP_STORE_GAME_CHART_TYPES,
  DEFAULT_APP_STORE_GAME_COUNTRY_CODES,
  type AppStoreGameChartType,
} from '../src/lib/app-store-games/types';
...
function parseCliArgs(argv: string[]) {
  ...
  return {
    ...
    chartTypes: parseEnumList(chartsArg, APP_STORE_GAME_CHART_TYPES) ?? [...APP_STORE_GAME_CHART_TYPES],
    countryCodes: parseCountryList(countriesArg) ?? [...DEFAULT_APP_STORE_GAME_COUNTRY_CODES],
  } satisfies {
    dryRun: boolean;
    snapshotHour: string;
    chartTypes: AppStoreGameChartType[];
    countryCodes: string[];
  };
}
...
async function fetchWithRetry(
  crawler: AppStoreGameChartsClient,
  countryCode: string,
...
async function main() {
  ...
  const crawler = new AppStoreGameChartsClient();
  const failures: Array<{ chartType: AppStoreGameChartType; countryCode: string; errorText: string }> = [];
  let successCount = 0;
  ...
  for (const chartType of options.chartTypes) {
    for (const countryCode of options.countryCodes) {
      try {
        ...
        if (!options.dryRun) {
          const snapshotId = await saveAppStoreGameChartSnapshot(snapshot);
          console.log(
  ...
  if (failures.length > 0) {
    throw new Error(`crawl-app-store-games completed with ${failures.length} failed chart snapshots`);
  }
}

main().catch((error) => {
  console.error('crawl-app-store-games failed:', error);
  process.exit(1);
});
